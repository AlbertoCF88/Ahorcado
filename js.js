console.log("Examen ahoracdo AlbertoCF");
...
//dentro del modal intentos
function intentosUser() {
    let inputIntentos = parseInt(document.getElementById('inputIntentos').value);
    intentos = inputIntentos;
    let validar = true;
    if (isNaN(intentos) == true || intentos <= 0) {
        modalInfo("Los intentos solo admiten un numero entero")
        validar = false;
    }
    document.getElementById("validarInput").onclick = function () {
        validarIntetos(validar)
    };
}
//dentro del modal intentos
function validarIntetos(validar) {
    if (validar == true) {
...
document.getElementById('intro_texto').focus();
document.getElementById('botonera').style.display = "none";

function inicio(ran) {
    //depende si viene del input o del random
    input = ran == undefined ? document.getElementById('intro_texto').value : ran;
    //si no ha escrito nada salir de la funcion
    if (input == "") {
        return;
    }
    //desabilitar botones de iniciar juego
    let btnIninciar = document.querySelectorAll('button[class*="ini"]')
    console.log("btn",btnIninciar)
    btnIninciar.forEach(btn => {
        btn.disabled = true;
        console.log("btn",btn)
    });
    //ocultar input
    document.querySelector('.controles').style.display = "none";
    input = limpiaTexto(input);
    console.log("input", input)
    let largo = input.length;
    for (let i = 0; i < input.length; i++) {
        if (input[i] == " ") {
            //buscar espacios
            palabraCifrada += input[i];
            let presentar = document.createElement("span")
            presentar.setAttribute('class', 'aparece espacio');
            setTimeout(() => {
                document.getElementById('salida').append(presentar);
            }, i * 500);
        }
        else {
            palabraCifrada += "-";
            let presentar = document.createElement("span")
            presentar.setAttribute('class', 'aparece guion');
            presentar.textContent = "-";
            setTimeout(() => {
                document.getElementById('salida').append(presentar);
            }, i * 500);
        }
        if (input[i] == input[largo - 1]) {
            //sacar botones una vez termine el bucle for
            setTimeout(() => {
                document.getElementById('botonera').style.display = "block";
                document.getElementById('botonera').classList.add('aparece');
            }, largo * 500);
...
function btnLetra(letra) {
    let inputSinTilde = limpiaLetra(input);
    //buscar letra
    let pala = "";
    let presentar = "";
    for (let i = 0; i < input.length; i++) {
        if (inputSinTilde[i] == letra) {
            pala += input[i];
            presentar += `<span>${input[i]}</span>`;
        } else if (inputSinTilde[i] == " ") {
            pala += palabraCifrada[i];
            presentar += `<span class="espacio"> </span>`;
        }
        else {
            pala += palabraCifrada[i];
            presentar += `<span>${palabraCifrada[i]}</span>`;
        }
    }
    //errores
    if (inputSinTilde.indexOf(letra) == -1) {
        if (listaErrores.includes(letra) == false) {
            intentos--;
            let span = document.createElement('span');
            span.textContent = `${letra}`
            span.setAttribute('class', 'let-err apareceRapido')
            document.getElementById('error').append(span)
            //document.getElementById('error').innerHTML += `<span class ="let-err">${letra}</span>`;
        } else {
            modalInfo("Ya has pulsado esa letra, por set tú no cuenta como intento");
        }
    ...
    //imprimir
    palabraCifrada = pala;
    document.getElementById('salida').innerHTML = presentar;
    //buscar una vez impreso
    let spanLetras = document.querySelectorAll('#salida span');
    for (let j = 0; j < spanLetras.length; j++) {
        spanLetras[j].textContent == letra ? spanLetras[j].setAttribute('class', 'amarillo') : null;
        spanLetras[j].textContent == "-" ? spanLetras[j].setAttribute('class', 'guion') : null;
    }
    finDelJUego(pala)
}

function finDelJUego(pala) {
    document.getElementById('intentos').innerHTML =
        `<span class="intentos" >Intentos: ${intentos}</span>`;
    if (input == pala) {
        modal("winer!!", "azul")
...
function limpiar() {
    ...
    intentos = 0;
    listaErrores = [];
    document.getElementById('intro_texto').value = "";
    ...
    document.getElementById('error').innerHTML = "";
     //habilitar botones de iniciar juego
     let btnIninciar = document.querySelectorAll('button[class*="ini"]')
     console.log("btn",btnIninciar)
     btnIninciar.forEach(btn => {
         btn.disabled = false;
...
//modal
function modal(mensaje, estilo) {
    let modal = document.getElementById("modal");
    modal.innerHTML += `<div class="modal">
            <div class="cardModal">
                <div class="cardBody ${estilo}">
                    <div class="pcentro">
                        <p>${mensaje} palabra: ${input}    <button class="btn" onclick="cerrar()">⛒</button></p> 
                    </div>
                </div>
            </div>
        </div>`
...
function modalInfo(palabra) {
    let modal = document.getElementById("infoModal");
    modal.innerHTML =
        ` <div class="modal" >
        <div class="cardModal">
            <div class="cardBody verde">
                <div class="pcentro">
                        <p>${palabra}</p> 
                </div>
            </div>
...
function modalDificultad() {
    ...
    modal.innerHTML += `<div class="modal">
            <div class="cardModal">
                <div class="cardBody negro">
                    <div class="pcentro">
                        <p>Elige la dificultad</p> 
                    </div>
                    <div>
                        <p>La palabra contiene: ${input.length} letras</p>
                    </div>
                    <div>
                        <label for="inputIntentos">Introduce número de intentos</label>
                        
                    </div>
                    <div style="margin-top: 20px">
                        <input min="1" type="number" name="inputIntentos" class="barraInput imputIntentos"
                            id="inputIntentos" onkeyup="intentosUser()" onchange="intentosUser()">
                        <button class="inicio btnWindows" id="validarInput" style="width: auto">Empezar</button>
                    </div> 
                </div>
            </div>
        </div>`
...
//btn ocultar errores
function ocultarErrores(_this) {
    if (_this.childNodes[1].className == "fa-regular fa-eye") {
        _this.childNodes[1].className = "fa-regular fa-eye-slash"
    } else {
        _this.childNodes[1].className = "fa-regular fa-eye"
    }

    if (_this.childNodes[1].className == "fa-regular fa-eye") {
        document.getElementById('idBtnError').classList.remove('btnErrorActive')
        document.getElementById('idBtnError').classList.add('btnError')
    } else {
        document.getElementById('idBtnError').classList.remove('btnError')
        document.getElementById('idBtnError').classList.add('btnErrorActive')
    }
...
function comprobarID() {
    ...
    console.log("lista_Ids", listaId)

    listaId.sort();
    for (let i = 0; i < listaId.length; i++) {
        if (listaId[i] == listaId[i + 1]) {
            console.log("¡ID repetido: ", listaId[i] + " !!!!!!!!!!");
            alert("ID repetido -> -> " + listaId[i]);
            return;
        }
    }
}
